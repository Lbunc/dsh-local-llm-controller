...
function Chat([object[]]$msgs, [int]$maxTok) {
  $body = @{ model = 'qwen35'; messages = $msgs; max_tokens = $maxTok; temperature = 1.0;
    top_k = 20; top_p = 0.95; min_p = 0.0; reasoning_effort = 'none' } | ConvertTo-Json -Depth 6
...
foreach ($N in 20, 22, 24) {
  ...
  try {
    ...
    $res = Chat @(@{ role = 'user'; content = 'Explain the water cycle in detail.' }) 64
    $t = $res.r.timings
    Write-Output "SHORT: eval=$([math]::Round($t.predicted_per_second,1))/s"
    ...
    $res = Chat $msgs26 700
    $t = $res.r.timings
    Write-Output "CTX26K: prompt=$($t.prompt_n)t gen=$($t.predicted_n)t eval=$([math]::Round($t.predicted_per_second,1))/s wall=$([math]::Round($res.wall,1))s"
    ...
    $res = Chat $msgs30 700
    $t = $res.r.timings
    Write-Output "CTX30K: prompt=$($t.prompt_n)t gen=$($t.predicted_n)t eval=$([math]::Round($t.predicted_per_second,1))/s wall=$([math]::Round($res.wall,1))s"

    # near-limit: 7 rounds + 200-token generation to check headroom stability
    $q31 = @{ role = 'user'; content = 'Add a brief conclusion paragraph.' }
    $msgs31 = $msgs + $q31
    $res = Chat $msgs31 200
    $t = $res.r.timings
    Write-Output "CTX31K: prompt=$($t.prompt_n)t gen=$($t.predicted_n)t eval=$([math]::Round($t.predicted_per_second,1))/s wall=$([math]::Round($res.wall,1))s"
  } catch {
