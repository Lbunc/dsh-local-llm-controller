...
foreach ($N in $Ns) {
  Write-Output "########## VISION-CONFIRM ncmoe=$N ##########"
  $res = Start-VisionServer $N
  if (-not $res) { Write-Output "N=$N SERVER NOT READY"; continue }
  Write-Output "N=$N idle: $(Get-Vram)"
  $shorts = @()
  for ($i = 1; $i -le 3; $i++) { $shorts += Test-Short; Start-Sleep -Milliseconds 400 }
  Write-Output "N=$N textShort: $($shorts -join ' / ') t/s"
  try { Write-Output "N=$N $(Test-Image $smallImg 'image/png' 'small')" } catch { Write-Output "N=$N small ERROR: $($_.Exception.Message)" }
  try { Write-Output "N=$N $(Test-Image $bigImg 'image/jpeg' 'big')" } catch { Write-Output "N=$N big ERROR: $($_.Exception.Message)" }
  try { Write-Output "N=$N $(Test-Image $bigImg 'image/jpeg' 'big#2')" } catch { Write-Output "N=$N big#2 ERROR: $($_.Exception.Message)" }
...
Write-Output '########## vision confirm done ##########'
