...
if (-not $ready) { Write-Output "C=$C SERVER NOT READY"; Stop-Process -Id $p.Id -Force -ErrorAction SilentlyContinue; exit 1 }
Write-Output "C=$C applied_n_ctx=OK idle: $(Get-Vram)"
...
try {
  $r = Invoke-RestMethod -Uri 'http://127.0.0.1:21113/v1/chat/completions' -Method Post -Headers $h -Body $body -TimeoutSec 600
  $t = $r.timings
  ...
  $util = 'n/a'
  if ($samples) {
    $vals = $samples | Select-Object -Last ([math]::Min($samples.Count, $genSec + 3)) | ForEach-Object { [int]$_ }
    $util = [math]::Round(($vals | Measure-Object -Average).Average, 0)
  }
  Write-Output "C=$C deep(rounds=$Rounds): prompt=$($t.prompt_n)t gen=$($t.predicted_n)t eval=$([math]::Round($t.predicted_per_second,1))/s gpu_util=$util%"
} catch {
  Write-Output "C=$C DEEP ERROR: $($_.Exception.Message)"
}
