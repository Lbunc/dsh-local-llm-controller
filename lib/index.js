...
export function apply(ctx) {
  ...
  /** Enumerate the .gguf file names inside one model folder ([] on any failure). */
  function listModelFiles(dir) {
    if (!dir) return []
    try {
      return fs.readdirSync(dir).filter((n) => /\.gguf$/i.test(n)).sort()
    } catch (e) {
      return []
    }
  }
  ...
  function resolveConfig() {
    ...
    for (const key of ['35b', '9b']) {
      ...
      const dir = o.dir ? resolvePath(o.dir) : defaultDir
      modCfg[key] = {
        dir,
        alias: o.alias || d.alias,
        providerKey: o.providerKey || d.providerKey,
        // chosen model file (filename inside `dir`); '' = auto-pick first non-mmproj
        file: typeof o.file === 'string' && o.file ? o.file : '',
        // live enumeration of the folder, published to the card form
   ...
   * file whose name contains "mmproj"; the model is the user-chosen file
   * (config `models.<key>.file`, a filename inside the folder) when it exists
   * in the folder, otherwise the first non-mmproj .gguf (previous behaviour —
   * folders with several quant files used to have no way to pick). Throws on
   * failure.
   */
  function resolveModelFiles(key) {
    ...
    const chosen = CFG.models[key].file
    const model = (chosen && ggufs.indexOf(chosen) >= 0) ? chosen : ggufs.find((n) => !/mmproj/i.test(n))
  ...
  ctx.on('settings/updated', (ns, next) => {
    ...
    if (action === 'start') {
      ...
      // enumeration published to the card form (models.<key>.files) follows the
      // folder the user just saved, then re-publish. The deep compare stops the
      // write-back (publishState → settings/updated → same config) from
      // ping-ponging.
