 ...
 * When the DSH provider section (llm-pi-ai.providers) is missing the plugin's
 * provider keys, they are created automatically (bootstrap) — no manual
 * provider editing needed.
 */
...
export function apply(ctx) {
   ...
   * providers are left untouched. Called at boot (with a delayed retry) and
   * again on every start; idempotent. Persisted to settings.yaml.
   *
   * Timing note: `llm-pi-ai`'s namespace registration rides a deferred
   * `ctx.inject()` nested fiber, so during OUR apply it may not be visible
   * yet — in that case the early call bails and the retry/start-time calls
   * land after pi-ai is fully wired.
   */
  function ensureProviders(attempt) {
    try {
      const desc = ctx.settings.describe().find((d) => d.ns === CFG.settingsNs)
      if (!desc) {
        note('bootstrap: namespace ' + CFG.settingsNs + ' not registered (attempt ' + (attempt || 0) + ')')
        return
      }
      // Fresh installs have no stored section at all (or no providers dict) —
      // build a minimal one so the providers can be created from scratch.
      ...
      for (const key of ['35b', '9b']) {
        ...
        if (!M || !mc || providers[mc.providerKey]) continue
        ...
        created.push(mc.providerKey)
      }
      if (!created.length) return
      ctx.settings.replace(CFG.settingsNs, section)
        .then(() => console.log('[local-llm] bootstrap: created providers ' + created.join(', ') + ' in ' + CFG.settingsNs + ' (configurable-provider directory updates live — no restart needed)'))
   ...
   * CURRENT effective config: baseURL port, apiKey → apiKeyEnv presence, and
   * the preset's contextWindow. Runs on every ready transition — changing the
   * card's port/key just needs a stop+start, and the configurable-provider
   * directory updates live (no DSH restart; the login message is history).
   */
  function syncProviderConfig() {
    ...
    try {
      ...
      if (CFG.apiKey && !provider.apiKeyEnv) { provider.apiKeyEnv = 'DSH_LOCAL_LLM_KEY'; changes.push('apiKeyEnv') }
      else if (!CFG.apiKey && provider.apiKeyEnv) { delete provider.apiKeyEnv; changes.push('no-auth') }
  ...
  function start(m, mo, p) {
    ...
    // Re-ensure providers before every launch (idempotent): covers the boot
    // ordering races AND any provider the user removed while DSH was up.
    ensureProviders()
    if (!CFG.llamaDir) {
      status = 'error'
  ...
  ensureProviders()
  // Retry once after a short delay: llm-pi-ai's namespace registration can
  // land after our own apply (deferred ctx.inject fiber), which would make
  // the immediate attempt above bail invisibly.
  ctx.timer.timeout(() => ensureProviders(1), 2000)
