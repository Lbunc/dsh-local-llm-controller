...
export function apply(ctx) {
  ...
  function syncProvidersToDsh() {
    try {
      ...
      if (!models.length) {
        note('add-providers: no slot has a chosen model file yet; nothing written')
        return
      }
      ...
      ctx.settings.replace(CFG.settingsNs, section)
        .then(() => console.log('[local-llm] add-providers: wrote ' + PROVIDER_KEY + ' (' + models.length + ' model(s)) into ' + CFG.settingsNs + (stale.length ? '; removed stale ' + stale.join(', ') : '') + ' (models page updates live)'))
